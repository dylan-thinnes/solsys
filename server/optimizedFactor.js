...
const Prime = new (function Prime () {
	...
	this.getFactors = function (number, callback) {
		...
		/*else cp.exec(msievePath + number.toString()).stdout.on("data", this.parseFactorsOutput.bind(this, number, callback));*/
			...
			console.log(this.msieveQueueCeilingIndex);
	...
	this.parsePrimecountOutput = function (stdout) {
		...
		this.piXHistory[stdout[0]] = stdout[1]; 
	...
	this.getPiX = function (number, callback) {
		...
		if (number <= 1) process.nextTick(callback.bind(this, 1));
		else if (this.piXHistory[number] !== undefined) process.nextTick(callback.bind(this, this.piXHistory[number]));
		else if (number > 9999999999999) {
			//console.log(logintPath + "--Li " + number + "\n"); cp.exec(logintPath + "--Li " + number + "\n").stdout.on("data", this.parsePiXOutput.bind(this, number, callback));
			...
			//console.log("using logint", "Write returns: ", this.logintProcess.stdin.writable, this.logintProcess.stdout.readable);
		} else if (number >= PIXDEPTH) {
			//cp.exec(primecountPath + number + "\n").stdout.on("data", this.parsePiXOutput.bind(this, number, callback));
			...
			//console.log("using primecount", "Write returns: ", this.logintProcess.stdin.writable, this.logintProcess.stdout.readable);
		} else process.nextTick(callback.bind(this, null));
	}
	...
	this.close = function () {
		...
		this.logintProcess.kill();
		this.primecountProcess.kill();
	}
})();
exports.Prime = Prime;
...
Factor.prototype.deepClone = function () {
	if (this.value === 1) {
		...
		var currClone = "{\"value\": \"" + this.value + "\", \"isPrime\": " + this.isPrime + ", \"power\": " + (this.power === 1 ? 1 : this.power.deepClone());
		if (this.isPrime === true && this.piX !== 1 && this.piX !== null && this.piX !== undefined) currClone += ", \"piX\": " + this.piX.deepClone();
		else if (this.piX === null) currClone += ", \"piX\": \"c\""; //currClone += ", \"piX\": " + (this.piX !== undefined ? this.piX : "\"undefined\"");
